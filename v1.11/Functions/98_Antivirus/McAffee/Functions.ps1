...
Process {
	...
	Stop-Service -Name McAfeeFramework -Force -ErrorAction SilentlyContinue

    Write-Verbose "Delete Registry Settings..."
	Remove-ItemProperty -Path "HKLM:\SOFTWARE\Network Associates\ePolicy Orchestrator\Agent" -Name "AgentGUID" -ErrorAction SilentlyContinue
	Remove-ItemProperty -Path "HKLM:\SOFTWARE\Network Associates\ePolicy Orchestrator\Agent" -Name "MACADDRESS" -ErrorAction SilentlyContinue
	Remove-ItemProperty -Path "HKLM:\SOFTWARE\Wow6432Node\Network Associates\ePolicy Orchestrator\Agent" -Name "AgentGUID" -ErrorAction SilentlyContinue
	Remove-ItemProperty -Path "HKLM:\SOFTWARE\Wow6432Node\Network Associates\ePolicy Orchestrator\Agent" -Name "MACADDRESS" -ErrorAction SilentlyContinue
