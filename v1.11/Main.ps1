...
.NOTES
    ...
    Link:	www.hornpa.de
    History:
	2017-05-15 - v1.11 - Bug fixing "Event logs could not deleted" (PHo)
	2017-05-12 - v1.10 - Bug fixing detection language (PHo)
    2017-05-04 - v1.09 - Bug fixing from security update april 2017 (CVE-2017-0160), unable shutdown Computer with PowerShell. Some code cleanup (PHo)
    2017-05-XX - v1.08 - Add new Function ReArm OS and ReArm Office (PHo)
...
#-----------------------------------------------------------[Main-Initialisations]------------------------------------------------------------
	...
	$Language = (Get-WmiObject -Class Win32_OperatingSystem).MUILanguages
	...
	IF ($scriptDebug -like 0)
	{
		IF (($Personality.StringData.DiskMode -like "P*") -or ($Personality.ArdenceData.DiskMode -like "P"))
		{
			$Msg = 	"Disk befindet sich im Private Mode" + [System.Environment]::NewLine + `
					"Das Skript wird weiter ausgefuehrt!"
